...
class StartFragment : Fragment() {
    ...
    fun checkRequiredPermissions(promptForNotGranted:Boolean = false){
        ...
        if(notGrantedPermissions.isEmpty()){
            removeMissingRequirement("Permission " + Manifest.permission.ACCESS_BACKGROUND_LOCATION.replace("android.permission.", "") + " not granted")
    ...
    fun checkBackgroundLocationAccessPermission(promptForNotGranted:Boolean = false):Boolean{
        ...
        if(promptForNotGranted && !isGranted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
