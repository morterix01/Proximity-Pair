...
class BluetoothLeScanService () : IBluetoothLeScanService, ScanCallback() {
    ...
    private var _scanRetryRunnable: Runnable? = null
    private var _scanRetryCount = 0
    private val _maxScanRetries = 3
    ...
    override fun startScanning(){
        if(_scanning){
            Log.d(_logTag, "startScanning ignored, scanner already active")
            return
        }

        if(_bluetoothAdapter?.isEnabled != true){
            Log.d(_logTag, "Cannot start scan, bluetooth adapter is disabled")
            return
        }

        if(_bluetoothLeScanner == null && _bluetoothAdapter != null){
            _bluetoothLeScanner = _bluetoothAdapter!!.bluetoothLeScanner
        }
    ...
    private fun scheduleScanRetry(errorCode: Int){
        val retriable = errorCode == SCAN_FAILED_ALREADY_STARTED ||
        ...
        if(!retriable || _scanRetryCount >= _maxScanRetries){
            return
        }

        _scanRetryCount += 1
        val retryDelayMs = 500L * _scanRetryCount
        clearPendingScanRetry()
        _scanRetryRunnable = Runnable {
            startScanning()
        }
        _scanHandler.postDelayed(_scanRetryRunnable!!, retryDelayMs)
    }

    private fun clearPendingScanRetry(){
        _scanRetryRunnable?.let { _scanHandler.removeCallbacks(it) }
        _scanRetryRunnable = null
    }
}